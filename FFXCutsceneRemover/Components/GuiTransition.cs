...
namespace FFXCutsceneRemover
{
    class GuiTransition : Transition
    {
        ...
        public override void Execute(string defaultDescription = "")
        {
            ...
            if (base.memoryWatchers.Dialogue1.Current == 63 && Stage == 0)
            {
                process.Suspend();
                
                new Transition { EncounterMapID = 27, EncounterFormationID = 0, ScriptedBattleFlag1 = 1, ScriptedBattleFlag2 = 1, EncounterTrigger = 2, Description = "Sinspawn Gui 1", ForceLoad = false }.Execute();
            ...
            else if (base.memoryWatchers.HpEnemyA.Current == 12000 && Stage == 1)
            {
            ...
            else if (base.memoryWatchers.BattleState2.Current == 0 && Stage == 2)
            {
                ...
                new Transition { RoomNumber = 247, Storyline = 865, EncounterMapID = 29, EncounterFormationID = 0, ScriptedBattleFlag1 = 0, ScriptedBattleFlag2 = 1, EncounterTrigger = 2, FormationSwitch = formations.PreGui2, Description = "Sinspawn Gui 2", ForceLoad = false }.Execute();

                Stage += 1;

                process.Resume();
            }
            else if (base.memoryWatchers.Menu.Current == 1 && Stage == 3)
            {
                process.Suspend();
